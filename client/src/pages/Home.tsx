...
export default function Home() {
  ...
  return <div className="console-app">
    <aside className={`console-sidebar ${mobileNav ? "open" : ""}`}>
      ...
      <div className="sidebar-footer"><div className="footer-line"><Activity size={14} /><span>DATA PLANE</span><StatusPill status="ready" /></div><div className="footer-line muted"><span>AUTHENTICATED AS</span></div><div className="account-line"><div className="avatar">{(user.name ?? user.email ?? "O").slice(0, 1).toUpperCase()}</div><div><strong>{user.name ?? "Operator"}</strong><small>{user.email ?? "Session identity"}</small></div></div><button className="text-button" onClick={() => logout()}>End session <ArrowUpRight size={14} /></button></div>
    </aside>
